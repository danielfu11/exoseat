...
#define CPU_FREQ        90U
#define TIMER0_PERIOD   500000U // in microseconds
...
__interrupt void cpu_timer2_isr(void)
{
    ...
    saturation = feedback;
    DCL_runClamp_C2(&saturation, pid_controller.Umax, pid_controller.Umin); //converts saturation to 1 or 0

    control_output = DCL_runPID_C3(&pid_controller, reference, feedback, saturation);
    ...
    if(((state == MOVING_UP) || (state == STARTUP_UP)) && (is_hall_prox_on_latch == false))
    {
