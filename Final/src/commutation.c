...
void motor_on(direction_e dir)
{
    ...
    // Commutate
    phase_drive_s drive_state = next_commutation_state(dir, hall_state, true);

    // After commutating twice, speed should be measured correctly
    int cnt = 0;
    while (cnt < 2)
    {
        if (new_hall_state)
        {
            ...
            hall_state = read_hall_states();
            drive_state = next_commutation_state(dir, hall_state, false);
            new_hall_state = false;
            cnt++;
        }
    }

    // Enable control system since speed is measured correctly
