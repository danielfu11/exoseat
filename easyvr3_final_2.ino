...
**
  ...
  Edited by Adam Smrekar for Exo-Seat Project
...
const int btn_pressed_pin      = 2;
const int wc_stop_pin          = 10;
const int wc_bring_me_down_pin = 3;
const int wc_pull_me_up_pin    = 4;
const int vc_stop_pin          = 7;
const int vc_bring_me_down_pin = 6;
const int vc_pull_me_up_pin    = 5;
const int num_group_attempts   = 4;
...
bridge:
    ...
    // Interrupt from Wired Controller
//      pinMode(interrupt_pin, INPUT_PULLUP);
//      attachInterrupt(digitalPinToInterrupt(interrupt_pin), WiredControllerInput, RISING);
    // Wired Controller Pins
    pinMode(btn_pressed_pin, INPUT);
    pinMode(wc_stop_pin, INPUT);
    pinMode(wc_bring_me_down_pin, INPUT);
    pinMode(wc_pull_me_up_pin, INPUT);
    // Voice Control Pins
    pinMode(vc_stop_pin, OUTPUT);
    pinMode(vc_bring_me_down_pin, OUTPUT);
    pinMode(vc_pull_me_up_pin, OUTPUT);
...
bool checkMonitorInput()
{
    ...
    {
        //int16_t num = readNum();
        pcSerial.println(F("Train Command: HEY_FELIX"));
        easyvr.stop();
        easyvr.trainCommand(0, 0);
        while (!easyvr.hasFinished());
    }
    if (rx == 'y')
    {
        //int16_t num = readNum();
        pcSerial.println(F("Train Command: PULL_ME_UP"));
        easyvr.stop();
        easyvr.trainCommand(1, 1);
        while (!easyvr.hasFinished());
    }
    if (rx == 'u')
    {
        //int16_t num = readNum();
        pcSerial.println(F("Train Command: BRING_ME_DOWN"));
        easyvr.stop();
        easyvr.trainCommand(1, 2);
        while (!easyvr.hasFinished());
    }
    if (rx == 'v')
    {
        //int16_t num = readNum();
        pcSerial.println(F("Train Command: STOP"));
        easyvr.stop();
        easyvr.trainCommand(1, 3);
        while (!easyvr.hasFinished());
    }
...
void WiredControllerInput()
{
    if(digitalRead(btn_pressed_pin) == HIGH)
    {
        pcSerial.println(F("In wired controller "));
        if(digitalRead(wc_pull_me_up_pin) == HIGH)
        {
            pcSerial.println(F("WC Command = Pull Me Up "));
            easyvr.playSound(0, EasyVR::VOL_FULL);
            easyvr.playSound(0, EasyVR::VOL_FULL);
            easyvr.playSound(0, EasyVR::VOL_FULL);
            easyvr.playSound(0, EasyVR::VOL_FULL);
        }
        else if(digitalRead(wc_bring_me_down_pin) == HIGH)
        {
            pcSerial.println(F("WC Command = Bring Me Down "));
            easyvr.playSound(0, EasyVR::VOL_FULL);
            easyvr.playSound(0, EasyVR::VOL_FULL);
            easyvr.playSound(0, EasyVR::VOL_FULL);
        }
        else if(digitalRead(wc_stop_pin) == HIGH)
        {
            pcSerial.println(F("WC Command = Stop "));
            easyvr.playSound(0, EasyVR::VOL_FULL);
            easyvr.playSound(0, EasyVR::VOL_FULL);
        }
    }
...
//              digitalWrite(SS, HIGH);
              spiTest();
              pcSerial.println('S');
  //          digitalWrite(vc_stop_pin, HIGH);
  //          delay(100);
  //          digitalWrite(vc_stop_pin, LOW);
          }
          else if(idx == 1 && group == 1)
          {
              easyvr.playSound(0, EasyVR::VOL_FULL);
              easyvr.playSound(0, EasyVR::VOL_FULL);
              easyvr.playSound(0, EasyVR::VOL_FULL);
  ...
  //          SPI.transfer('\n');
  //          digitalWrite(SS, HIGH);
  //          digitalWrite(vc_bring_me_down_pin, HIGH);
  //          delay(100);
  //          digitalWrite(vc_bring_me_down_pin, LOW);
          }
          else if(idx == 2 && group == 1)
          {
              easyvr.playSound(0, EasyVR::VOL_FULL);
              easyvr.playSound(0, EasyVR::VOL_FULL);
              easyvr.playSound(0, EasyVR::VOL_FULL);
  ...
  //          SPI.transfer('\n');
  //          digitalWrite(SS, HIGH);
  //          digitalWrite(vc_pull_me_up_pin, HIGH);
  //          delay(100);
  //          digitalWrite(vc_pull_me_up_pin, LOW);
          }
          else
          {
  //          digitalWrite(vc_stop_pin, LOW);
  ...
  //          digitalWrite(vc_pull_me_up_pin, LOW);
      ...
      else // errors or timeout
      {
          ...
          if (err >= 0)
          {
              ...
              if(group_attempts == num_group_attempts)
              {
                  pcSerial.println(F("Max Attempts Used."));
                  easyvr.playSound(0, EasyVR::VOL_FULL);
                  group_attempts = 0;
                  group = 0;
              }
